...
export default function BookRide() {
  ...
  return (
    <StripeProvider
      publishableKey={key!}
      merchantIdentifier="merchant.mydrive.com" // required for Apple Pay
      urlScheme="myapp" // required for 3D Secure and bank redirects
    >
      <RideLayout title="Book Ride" snapPoints={["75%, 95%"]}>
        <>
          ...
          <View className="flex flex-col w-full items-start justify-center mt-5">
            <View className="flex flex-row items-center justify-start mt-3 border-t  border-general-700 w-full py-3">
              <Image source={icons.to} className="w-6 h-6" />
              <Text className="text-lg font-JakartaRegular ml-2">{userAddress}</Text>
            </View>

            <View className=" mt-5 flex flex-row items-center justify-start border-general-700 w-full py-3">
